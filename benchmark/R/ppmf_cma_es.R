...
library(checkmate)
library(magrittr)
source(here::here("benchmark", "R", "cmaesr-utils.R"))
...
rb_ipop_cma_esr_ppmf = function(
  ...
  restart.triggers = list() # list("conditionCov", "noEffectCoord", "noEffectAxis", "tolX", "indefCovMat", "lastIts")
  ...
  for (run in 0:max.restarts) {
    ...
    if (run == 0) {
      lambda = getCMAESParameter(control, "lambda", 4)
