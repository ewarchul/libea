...
const auto tol_x =
    criterium<P, S>{"[TolX] standard deviation below tolerance in all coordinates", [](const auto& p, const auto& s) {
                      const auto std_cond = (s.eigen_vals_diag_.diagonal().array() < p.tolx_).sum() == p.dim_;
...
const auto tol_fitness = criterium<P, S>{"[TolFitness] optimal function value approximated", [](const auto& p, const auto& s) {
                                           return std::abs(s.best_so_far_fitness < p.target_fitness_) < p.tol_fitness_;
                                         }};
...
const auto predefined_termination_criteria =
    termination_criteria<Parameters, Solutions>{.criteria_ = {max_fevals<Parameters, Solutions>, tol_x<Parameters, Solutions>}};
